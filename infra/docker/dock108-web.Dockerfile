...
# We run install after sources are in place so pnpm can link workspace packages
# correctly and ensure package bins (like next) are available.
...
COPY --from=builder /app/apps/dock108-web/node_modules ./node_modules
