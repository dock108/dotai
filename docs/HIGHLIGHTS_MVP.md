...
1. **No User Accounts**
   - Anonymous requests only (no saved preferences)
   - Cannot track user history or favorites
   - No personalized recommendations

2. **Limited Content Mix Control**
   - Content mix is inferred from query text
   - No fine-grained control over highlight vs blooper ratios
   - Cannot specify exact video types beyond keywords

3. **No Real-time Updates**
   - Playlists are static once created
   - No automatic refresh when new videos are published
   - Staleness logic is time-based, not event-based

4. **Single Sport Focus**
   - Primarily designed for single-sport requests
   - Multi-sport playlists are possible but not optimized
   - No cross-sport comparisons or analysis

5. **No Video Quality Filtering**
   - Cannot filter by resolution (720p, 1080p, 4K)
   - Relies on YouTube's relevance ranking
   - May include lower-quality fan uploads
